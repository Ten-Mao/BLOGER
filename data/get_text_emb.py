...
def preprocess_text(args):
    print("Process text data: title and description")
...
def generate_item_embedding(args, item_text_list, tokenizer, model, word_drop_ratio=-1):
    ...
    assert min(items) == 0, "Index should start from zero"
    ...
    while start < len(order_texts):
        ...
        for sentences in field_texts:
            # sentences: titles/descriptions of batch_size items
            sentences = list(sentences)  # tuple -> list
            # print(sentences)
            ...
            with torch.no_grad():
                outputs = model(
                    input_ids=encoded_sentences.input_ids,  # [batch_size, token_num]
                    attention_mask=encoded_sentences.attention_mask,
                )
            # last_hidden_state: Sequence of hidden-states at the output of the last layer of the model
            # [batch_size, token_num, hidden_dim] * [batch_size, token_num, 1]
...
def parse_args():
    ...
    parser.add_argument(
        "--plm_checkpoint", type=str, default="/data/baiyimeng/Llama-2-7b-hf"
    )
